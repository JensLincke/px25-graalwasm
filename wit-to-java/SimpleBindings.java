...
public class SimpleBindings {
    ...
    public float addFloat(float x, float y) {
        float result = wasmModule.getMember("exports_docs_adder_simple_add_float").execute(x, y).asFloat();
    ...
    public java.util.List<Float> getStatisticsListInt(java.util.List<java.util.List<Integer>> x) {
        // Lowering parameter 'x' to wasm memory
        int temp0 = allocate(8, 4);
        int temp1 = allocate(x.size() * 8, 4);
        for (int temp3 = 0; temp3 < x.size(); temp3++) {
            java.util.List<Integer> temp4 = x.get(temp3);
        int temp5 = allocate(temp4.size() * 4, 4);
        for (int temp7 = 0; temp7 < temp4.size(); temp7++) {
            Integer temp8 = temp4.get(temp7);
        ByteBuffer temp9 = ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN);
        temp9.putInt(temp8);
        writeBytes(temp5 + temp7 * 4, temp9.array());
        }
        ByteBuffer temp10 = ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN);
        temp10.putInt(temp5).putInt(temp4.size());
        writeBytes(temp1 + temp3 * 8, temp10.array());
        }
        ByteBuffer temp11 = ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN);
        temp11.putInt(temp1).putInt(x.size());
        writeBytes(temp0, temp11.array());
        int result_ptr = allocate(8, 4);
        wasmModule.getMember("exports_docs_adder_simple_get_statistics_list_int").executeVoid(temp0, result_ptr);
        // Lifting result from wasm memory
        byte[] temp13 = readBytes(result_ptr, 8);
        ByteBuffer temp14 = ByteBuffer.wrap(temp13).order(ByteOrder.LITTLE_ENDIAN);
        int temp15 = temp14.getInt(0);
        int temp16 = temp14.getInt(4);
        java.util.List<Float> temp12 = new java.util.ArrayList<>(temp16);
        for (int temp17 = 0; temp17 < temp16; temp17++) {
        byte[] temp19 = readBytes(temp15 + temp17 * 4, 4);
        ByteBuffer temp20 = ByteBuffer.wrap(temp19).order(ByteOrder.LITTLE_ENDIAN);
        float temp18 = temp20.getFloat(0);
            temp12.add(temp18);
        }
        free(temp15);        free(result_ptr);
        free(temp5);
        free(temp1);
        free(temp0);
